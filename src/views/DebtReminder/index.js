...
const DebtReminderPage = () => {
  ...
  const handleSubmit = async (values) => {
    const result = await api.post('debtReminder', values, token)
    if (result.isSuccess) {
      message.success('Debt reminder was created successfully!')
    ...
    <Wrapper>
      <Form
        ...
        {selectRecAccMode === SELECT_REC_ACC_MODE.NEW
          ...
          : null}
        <h1>Debt reminder information</h1>
        <br />
        <Form.Item
          label={'Enter debt amount: '}
          name='amount'
          rules={[
            {
              required: true,
              message: 'Please enter the amount of debt!'
            }
          ]}
        >
          <Input
        ...
        <Form.Item
          label={'Enter debt content: '}
          name='content'
          rules={[
            {
              required: true,
              message: 'Please enter the content of debt!'
            }
          ]}
        >
          <Input placeholder='Debt content' />
        </Form.Item>
        <Form.Item>
          <FormFooter>
...
export default DebtReminderPage
